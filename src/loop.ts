import { addEvent, addUsage, claimNextTask, getProject, insertIteration, latestAwaitingManual, listDirectives, listIterations, markNextTaskWaiting, markRunningTasks, nextIterationNumber, setProjectStatus, setTaskStatus, updateIteration } from "./db.ts";
...
import type { IterationRecord, ProjectRecord, ReviewResult, TaskRecord } from "./types.ts";
...
export class LoopController {
  ...
  stop(projectId: string, finalStatus: ProjectRecord["status"] = "STOPPED"): boolean {
    ...
    markRunningTasks(projectId, "PAUSED");
    setProjectStatus(projectId, finalStatus);
    addEvent(projectId, "run.stop_requested", { finalStatus });
    return true;
  }
  ...
  private async deliverReviewedCheckpoint(project: ProjectRecord, iteration: IterationRecord, review: ReviewResult, projectComplete: boolean): Promise<boolean> {
    ...
    try {
      addEvent(project.id, "github.checkpoint_started", { iteration: iteration.number, branch: project.definition.githubIntegration.workBranch });
  ...
  async run(projectId: string, requestedIterations: number): Promise<void> {
    ...
    try {
      for (let n = 0; n < requestedIterations; n += 1) {
        ...
        addEvent(projectId, "supervisor.started", { cycle: n + 1, workspace: project.workspacePath });
        ...
        addEvent(projectId, "supervisor.decided", { action: decision.recommendedAction, task: decision.taskTitle, summary: decision.reasoningSummary });

        if (decision.recommendedAction === "ASK_USER") {
          const waitingTask = markNextTaskWaiting(projectId);
          setProjectStatus(projectId, "NEEDS_HUMAN");
          addEvent(projectId, "supervisor.needs_human", { question: decision.userQuestion, taskId: waitingTask?.id || "", task: waitingTask?.title || "" });
          return;
        }
        ...
        const task: TaskRecord | null = claimNextTask(projectId, iteration.id);
        addEvent(projectId, "iteration.started", { iteration: iteration.number, task: decision.taskTitle, plannedTask: task?.title || "", stageIndex: task?.stageIndex ?? null, taskIndex: task?.taskIndex ?? null });
        ...
        if (project.executorMode === "manual") {
          ...
          if (task) setTaskStatus(task.id, "WAITING");
          setProjectStatus(projectId, "NEEDS_HUMAN"); addEvent(projectId, "executor.manual_required", { iterationId: iteration.id, taskId: task?.id || "" }); return;
        }

        addEvent(projectId, "executor.started", { iteration: iteration.number, task: decision.taskTitle, workspace: project.workspacePath });
        ...
        addEvent(projectId, "executor.completed", { iteration: iteration.number, durationMs: executorRun.usage.durationMs, threadId: executorRun.threadId });

        addEvent(projectId, "reviewer.started", { iteration: iteration.number });
        ...
        addEvent(projectId, "reviewer.completed", { iteration: iteration.number, score: review.score, status: review.status, summary: review.reasoningSummary });
        ...
        if (task) setTaskStatus(task.id, review.status === "PASS" ? "DONE" : review.requiresHumanDecision ? "WAITING" : "ATTENTION");
        addEvent(projectId, "iteration.completed", { iteration: iteration.number, score: review.score, decision: loopDecision, taskId: task?.id || "" });
      ...
      if (controller.signal.aborted) return;
      markRunningTasks(projectId, "ATTENTION");
      const message = error instanceof Error ? error.message : String(error);
      setProjectStatus(projectId, "ERROR"); addEvent(projectId, "run.error", { message }); throw error;
    } finally {
      ...
      addEvent(projectId, "run.finished", { status: getProject(projectId)?.status || "UNKNOWN" });
    }
  }
  ...
  async submitManualResult(projectId: string, result: string): Promise<ReviewResult> {
    ...
    addEvent(projectId, "reviewer.started", { iteration: iteration.number, manual: true });
    ...
    addEvent(projectId, "reviewer.completed", { iteration: iteration.number, score: review.score, status: review.status, manual: true });
